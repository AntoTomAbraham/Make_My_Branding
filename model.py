...
x = df.iloc[:,[0,2,3,4,5,8,9,10]] #independent
y = df.iloc[:,-1] #dependent

#encoding categorical data
x_d = pd.get_dummies(x, columns=["Location","NatureOfLocation","LocationType","Category","AgeGroup","PPI","Light"])
...
newData = pd.get_dummies(pd.DataFrame({'Location':['Kolkata'],'NatureOfLocation':['CBD'],'LocationType':['Urban'],'Category':['Gantry'],'AgeGroup':['Youth'],'Size(sqft)':[200],'PPI':['Medium'],'Light':['No']}))
