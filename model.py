...
x = df.iloc[:,[0,2,3,5,8,9,10]] #independent
y = df.iloc[:,-1] #dependent

#encoding categorical data
x_d = pd.get_dummies(x, columns=["Location","NatureOfLocation","LocationType","AgeGroup","PPI","Light"])
...
newData = pd.get_dummies(pd.DataFrame({'Location':['Hyderabad'],'NatureOfLocation':['CBD'],'LocationType':['Urban'],'AgeGroup':['Youth'],'Size(sqft)':[200],'PPI':['Medium'],'Light':['No']}))
