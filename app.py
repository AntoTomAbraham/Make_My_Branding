...
def predict():
    ...
    newData = pd.get_dummies(pd.DataFrame({'State':[request.args.get('State')],'Location':[request.args.get("Location")],'NatureOfLocation':[request.args.get("NOL")],'LocationType':[request.args.get("LocType")],'Category':[request.args.get("Category")],'AgeGroup':[request.args.get("AgeGroup")],'Size(sqft)':[int(request.args.get("Size"))],'PPI':[request.args.get("PPI")],'Light':[request.args.get("Light")]}))
